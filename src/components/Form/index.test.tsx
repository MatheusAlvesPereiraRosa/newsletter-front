...
const renderForm = (setAlert: jest.Mock) =>
  render(
    <BrowserRouter> {/* Wrap with BrowserRouter */}
      <AlertContext.Provider value={{ alert: { isVisible: false, type: '', message: '' }, setAlert }}>
        <Form />
...
describe('Form Component', () => {
  ...
  test('submits form data successfully and displays success alert', async () => {
    ...
    fireEvent.change(screen.getByTestId('business'), { target: { value: 'Acme Corp' } });
  ...
  test('displays error alert when the form submission fails', async () => {
    ...
    mock.onPost('http://localhost:3000/user/create').reply(400, { message: 'Falha ao registrar!' });
